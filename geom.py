...
class grid():
    ...
    def plot(self): #Plots all boxes on the grid
        ...
        handles.append(patch)
        plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.10),ncol=3, fancybox=True, shadow=True,fontsize=11,handles=handles)
...
g.ax.view_init()
plt.tight_layout()
plt.savefig("mecha_design.svg")
x_ticks=g.ax.get_xticks()
y_ticks=g.ax.get_yticks()
z_ticks=g.ax.get_zticks()
g.ax.view_init(elev=0, azim=-90)
g.ax.set_yticks([])
g.ax.get_yaxis().set_visible(False)
plt.savefig("front_viewXZ.pdf",bbox_inches='tight')
g.ax.get_yaxis().set_visible(True)
g.ax.set_yticks(y_ticks)
g.ax.view_init(elev=0, azim=0)
g.ax.set_xticks([])
g.ax.get_xaxis().set_visible(False)
plt.savefig("side_viewYZ.pdf",bbox_inches='tight')
g.ax.view_init(elev=90, azim=-90)
g.ax.get_xaxis().set_visible(True)
g.ax.set_xticks(x_ticks)
g.ax.set_zticks([])
g.ax.get_zaxis().set_visible(False)
plt.savefig("upper_viewXY.pdf",bbox_inches='tight')
